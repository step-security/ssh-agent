...
const index_crypto = __nccwpck_require__(6982);
const { homePath, sshAgentCmd, sshAddCmd, gitCmd } = __nccwpck_require__(6644);
...
try {
    (async () => {
        ...
        child_process.execFileSync(sshAddCmd, ['-L']).toString().trim().split(/\r?\n/).forEach(function(key) {
            ...
            const sha256 = index_crypto.createHash('sha256').update(key).digest('hex');
